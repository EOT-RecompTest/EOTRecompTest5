...
#pragma region System/Debug
    uint32_t GuestTimeoutToMilliseconds(big_endian<int64_t>* timeout);
    void KeQuerySystemTime(big_endian<uint64_t>* time);
    ...
    uint32_t XAudioGetVoiceCategoryVolumeChangeMask(uint32_t Driver, big_endian<uint32_t>* Mask);
    ...
    uint32_t ObReferenceObjectByHandle(uint32_t handle, uint32_t objectType, big_endian<uint32_t>* object);
    ...
    uint32_t RtlUnicodeToMultiByteN(char* MultiByteString, uint32_t MaxBytesInMultiByteString, big_endian<uint32_t>* BytesInMultiByteString, const big_endian<uint16_t>* UnicodeString, uint32_t BytesInUnicodeString);
    uint32_t RtlMultiByteToUnicodeN(big_endian<uint16_t>* UnicodeString, uint32_t MaxBytesInUnicodeString, big_endian<uint32_t>* BytesInUnicodeString, const char* MultiByteString, uint32_t BytesInMultiByteString);
    ...
    uint32_t XMsgInProcessCall(uint32_t app, uint32_t message, big_endian<uint32_t>* param1, big_endian<uint32_t>* param2);
    ...
    // Additional stubs
    uint32_t XexLoadImage(const char* path, uint32_t typeFlags, uint32_t minVersion, big_endian<uint32_t>* handle);
    ...
    uint32_t ExGetXConfigSetting(uint16_t Category, uint16_t Setting, void* Buffer, uint16_t SizeOfBuffer, big_endian<uint32_t>* RequiredSize);

#pragma endregion

#pragma region Threading
    uint32_t ExCreateThread(big_endian<uint32_t>* handle, uint32_t stackSize, big_endian<uint32_t>* threadId, uint32_t xApiThreadStartup, uint32_t startAddress, uint32_t startContext, uint32_t creationFlags);
    ...
    uint32_t KeDelayExecutionThread(uint32_t WaitMode, bool Alertable, big_endian<int64_t>* Timeout);
    ...
    uint32_t KeSetAffinityThread(uint32_t Thread, uint32_t Affinity, big_endian<uint32_t>* lpPreviousAffinity);
    ...
    uint32_t NtCreateSemaphore(big_endian<uint32_t>* Handle, XOBJECT_ATTRIBUTES* ObjectAttributes, uint32_t InitialCount, uint32_t MaximumCount);
    void KeInitializeSemaphore(XKSEMAPHORE* semaphore, uint32_t count, uint32_t limit);
    
    uint32_t NtCreateMutant(big_endian<uint32_t>* handle, void* objAttributes, uint32_t initialOwner);
    uint32_t NtReleaseMutant(Mutant* handle, big_endian<uint32_t>* previousCount);
    ...
    uint32_t NtCreateEvent(big_endian<uint32_t>* handle, void* objAttributes, uint32_t eventType, uint32_t initialState);
    ...
    uint32_t KeWaitForMultipleObjects(uint32_t Count, xpointer<XDISPATCHER_HEADER>* Objects, uint32_t WaitType, uint32_t WaitReason, uint32_t WaitMode, uint32_t Alertable, big_endian<int64_t>* Timeout);
    void NtWaitForMultipleObjectsEx();

    uint32_t KeWaitForSingleObject(XDISPATCHER_HEADER* Object, uint32_t WaitReason, uint32_t WaitMode, bool Alertable, big_endian<int64_t>* Timeout);
    uint32_t NtWaitForSingleObjectEx(uint32_t Handle, uint32_t WaitMode, uint32_t Alertable, big_endian<int64_t>* Timeout);
...
#pragma region Filesystem
    ...
    (
        big_endian<uint32_t>* FileHandle,
...
#pragma region Memory
    uint32_t NtAllocateVirtualMemory(uint32_t processHandle, big_endian<uint32_t>* baseAddress, uint32_t zeroBits, big_endian<uint32_t>* regionSize, uint32_t allocationType, uint32_t protect);
    uint32_t NtFreeVirtualMemory(uint32_t processHandle, big_endian<uint32_t>* baseAddress, big_endian<uint32_t>* regionSize, uint32_t freeType);
